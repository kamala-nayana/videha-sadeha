...
No author/title/date is inferred from Sadeha when a reliable Videha original exists.
"""
...
def page_texts(path):
    ...
    if not out:
        # Existing search HTMLs can be non-PDF-derived. Keep one whole-document page
        # rather than dropping the source from the cross-map.
...
def main():
    ...
    for path in paths:
        ...
        for r,nt,na in originals:
            ...
            matches.append({
                'sadeha_source_file':path.name,
                'sadeha_source_key':path.stem,
                'sadeha_issue':issue,
                ...
                'author':r.get('author'),'title':r.get('title'),'classification':r.get('classification'),
    ...
    scholar=[m for m in matches if m['scholar_candidate']]; new=[m for m in scholar if not m['already_published']]
    ...
    payload={
        'sadeha_source_html_files_scanned':len(paths),
        'sadeha_distinct_issue_numbers':len({issue_num(p) for p in paths if issue_num(p) is not None}),
        'source_files':[p.name for p in paths],
        'videha_inventory_entries':len(inv_rows),
        'high_confidence_reprint_matches_by_source':len(matches),
        'unique_videha_originals_rediscovered':len(unique_originals),
        'scholar_candidate_reprint_matches_by_source':len(scholar),
        'unique_scholar_candidate_originals_rediscovered':len(unique_scholar),
        'scholar_candidate_matches_not_yet_published_by_source':len(new),
        'unique_scholar_candidate_originals_not_yet_published':len(unique_new),
        'matches':matches,'new_scholar_candidates':new,
    }
    OUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding='utf-8')
    print(f"Sadeha/Videha map: {len(paths)} source HTML files; {len(unique_originals)} unique Videha originals; {len(unique_scholar)} Scholar candidates; {len(unique_new)} not yet published")
